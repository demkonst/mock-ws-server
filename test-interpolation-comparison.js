...
async function testInterpolationComparison() {
  console.log('🧪 Сравниваем интерполяцию операторов...');
  
  try {
    // 1. Тест с интерполяцией
    console.log('\n📤 Тест 1: Оператор с интерполяцией...');
    const test1Response = await fetch('http://localhost:3000/run', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        env: 'dev',
        operators: ['with_interpolation'],
        duration: 5
      })
    });
    ...
    console.log('\n📤 Тест 2: Оператор без интерполяции...');
    const test2Response = await fetch('http://localhost:3000/run', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        env: 'dev',
        operators: ['without_interpolation'],
        duration: 5
      })
    });
...
if (require.main === module) {
  testInterpolationComparison();
}

module.exports = { testInterpolationComparison }; 